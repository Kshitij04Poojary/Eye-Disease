...
export default function MultiStepPatientForm() {
  ...
  return (
    ...
    <div className="min-h-screen p-6 bg-gradient-to-br from-gray-50 to-blue-50">
      {toastMessage && (
        <div className={`fixed top-4 right-4 max-w-md p-4 rounded-lg shadow-xl transform transition-all duration-500 ${toastMessage.type === 'success' ? 'bg-emerald-500' : 'bg-red-500'
          } text-white`}>
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-gray-600">Please fill in the required information</p>
          </div>

          <div className="flex justify-between items-center mb-8">
            {['Patient Information', 'Left Eye', 'Right Eye'].map((stage, idx) => (
              <div key={idx} className="flex flex-col items-center w-1/3">
                <div className={`
                  w-12 h-12 rounded-full flex items-center justify-center mb-2
                  ${idx + 1 === step ? 'bg-blue-100 text-blue-600 ring-2 ring-blue-600' :
                    idx + 1 < step ? 'bg-blue-600 text-white' :
                      'bg-gray-100 text-gray-400'}
                `}>
                  {idx + 1 < step ? '✓' : idx + 1}
                </div>
                <span className={`text-sm ${idx + 1 === step ? 'text-blue-600 font-medium' : 'text-gray-500'}`}>
                  {stage}
                </span>
              </div>
            ))}
          ...
          <div className="mb-8">
            ...
            {step === 2 && (
              <div className="space-y-4">
                <h3 className="text-lg font-medium text-gray-900 mb-4">Left Eye Fundus Image</h3>
                <div className={`border-2 ${formData.leftEyeImage ? 'border-blue-500 bg-blue-50' : 'border-dashed border-gray-300'} rounded-lg p-6 text-center transition-colors`}>
                  <input
                    type="file"
                    id="left-eye-upload"
                    className="hidden"
                  ...
                  {!formData.leftEyeImage ? (
                    <label htmlFor="left-eye-upload" className="cursor-pointer">
                      ...
                      <span className="text-gray-600">Click to upload left eye image</span>
                      <p className="text-sm text-gray-500 mt-2">PNG, JPG, GIF up to 5MB</p>
                    </label>
                  ) : (
                    <div className="text-center">
                      <div className="text-blue-600 font-medium mb-2">Image Uploaded Successfully</div>
                      <div className="text-sm text-blue-500">{formData.leftEyeImage.name}</div>
                      <button
                        onClick={() => setFormData(prev => ({ ...prev, leftEyeImage: null }))}
            ...
            {step === 3 && (
              <div className="space-y-4">
                <h3 className="text-lg font-medium text-gray-900 mb-4">Right Eye Fundus Image</h3>
                <div className={`border-2 ${formData.rightEyeImage ? 'border-blue-500 bg-blue-50' : 'border-dashed border-gray-300'} rounded-lg p-6 text-center transition-colors`}>
                  <input
                    type="file"
                    id="right-eye-upload"
                    className="hidden"
                  ...
                  {!formData.rightEyeImage ? (
                    <label htmlFor="right-eye-upload" className="cursor-pointer">
                      <div className="mx-auto w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                        <svg className="w-8 h-8 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                        </svg>
                      </div>
                      <span className="text-gray-600">Click to upload right eye image</span>
                      <p className="text-sm text-gray-500 mt-2">PNG, JPG, GIF up to 5MB</p>
                    </label>
                  ) : (
                    <div className="text-center">
                      <div className="text-blue-600 font-medium mb-2">Image Uploaded Successfully</div>
                      <div className="text-sm text-blue-500">{formData.rightEyeImage.name}</div>
                      <button
                        onClick={() => setFormData(prev => ({ ...prev, rightEyeImage: null }))}
                        className="mt-4 px-4 py-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition-colors"
                      >
                        Change Image
                      </button>
