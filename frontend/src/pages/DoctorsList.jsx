...
import { Calendar, Clock, ArrowRight, Check, X, CheckCircle } from 'lucide-react';
...
const DoctorsList = () => {
  ...
  const fetchDoctors = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3000/api/mydoctor/getAllDoctors', {
        headers: {
          ...
          <div className="relative p-6">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex-shrink-0">
                <div className={`w-24 h-24 rounded-full ${getAvatarColor(doctor.name)} flex items-center justify-center text-white text-2xl font-bold`}>
              ...
              <div className="flex-grow">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-gray-600">
                    {doctor.aboutMe || `Dr. ${doctor.name} is a highly qualified healthcare professional specializing in ${doctor.specialization} with ${doctor.yearsOfExperience} years of experience in providing exceptional patient care.`}
                  </p>
                </div>
  ...
  const BookingFlow = ({ doctor, onClose }) => {
    ...
    const fetchDoctorId = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3000/api/v1/user/doctor-id/${email}`);
    ...
    const fetchPatientId = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3000/api/v1/user/patient-id/${email}`);
    ...
    const fetchTimeSlots = async (date) => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:3000/api/doctor/${doctorId}/${formattedDate}`);
        const data = await response.json();

        if (data.success) {
          const availableSlots = timeSlotsList.filter(slot =>
            data.data.timeSlots.includes(slot.id) && !bookedSlots.includes(slot.id)
          );
        ...
        <div className="bg-white rounded-2xl w-full max-w-2xl">
          <div className="p-6">
            <div className="relative">
              ...
              <div className="mb-8">
                <div className="flex items-center justify-center">
                  <div className="flex items-center">
                    <div className={`flex items-center justify-center w-8 h-8 rounded-full ${activeTab === 'date' ? 'bg-blue-500 text-white' : 'bg-gray-200'
                      }`}>
                      <Calendar className="w-4 h-4" />
                    </div>
                    <div className={`h-1 w-16 mx-2 ${activeTab === 'time' ? 'bg-blue-500' : 'bg-gray-200'
                      }`} />
                    <div className={`flex items-center justify-center w-8 h-8 rounded-full ${activeTab === 'time' ? 'bg-blue-500 text-white' : 'bg-gray-200'
                      }`}>
              ...
              {/* Time Slots Tab */}
              {activeTab === 'time' && !loading && !error && !bookingConfirmed && (
                <div className="space-y-6">
                    ...
                    <div key={period} className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {slots.map((slot) => (
                          <button
                            key={slot.id}
                            onClick={() => !isSlotBooked(slot.id) && setSelectedTimeSlot(slot)}
                            disabled={isSlotBooked(slot.id)}
                            className={`relative px-4 py-3 text-sm rounded-lg transition-all duration-200 ${isSlotBooked(slot.id)
                  ...
                  <div className="flex justify-between mt-8">
                    ...
                    {selectedTimeSlot && !isSlotBooked(selectedTimeSlot.id) && timeSlots.length > 0 && (
                      <button
              ...
              {bookingConfirmed && (
                <div className="flex flex-col items-center justify-center py-8">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900 mt-4">Booking Done Successfully!</h2>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4 max-w-4xl">
        {/* Header */}
        <h1 className="text-3xl font-bold text-gray-800 mb-8 text-center">Our Medical Specialists</h1>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className={`w-14 h-14 rounded-lg ${getAvatarColor(doctor.name)} flex items-center justify-center text-white font-bold text-xl shadow-sm`}>
                    {getInitials(doctor.name)}
                  </div>
                  <div className="flex flex-col">
                    <span className="font-semibold text-lg text-gray-800">{doctor.name}</span>
