...
const DoctorProfile = () => {
  ...
  const fetchDoctorProfile = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/mydoctor/profile/${email}`, {
        method: "GET",
        credentials: "include",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/mydoctor/profile/${user.email}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
            ...
            {!isEditing ? (
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-600">{formData.specialization || 'Not specified'}</p>
                </div>

                <div>
                  <h3 className="text-lg font-semibold text-gray-700 mb-2">Years of Experience</h3>
                  <p className="text-gray-600">{formData.yearsOfExperience || 'Not specified'}</p>
                </div>

                <div>
                  <h3 className="text-lg font-semibold text-gray-700 mb-2">About Me</h3>
                  <p className="text-gray-600 whitespace-pre-line">{formData.aboutMe || 'Not specified'}</p>
                </div>
